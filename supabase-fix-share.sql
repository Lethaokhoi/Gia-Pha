-- Chỉ phần CHIA SẺ (nếu đã tạo gia phả được nhưng hộp «Chia sẻ» báo lỗi gp_list_family_access)
-- Supabase → SQL Editor → Run → đợi 10 giây → F5 trang web
...
begin
  ...
  insert into public.gp_family_invites (family_id, email, invited_by) values (p_family_id, norm, auth.uid())
...
begin
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object(
      'family_id', i.family_id, 'family_name', f.name, 'invite_email', i.email
